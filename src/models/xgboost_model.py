"""XGBoost regression model for daily high temperature prediction."""
...
class XGBoostTemperatureModel:
    """XGBoost regression model for predicting daily high temperatures."""
    ...
    def __init__(self, model_dir: Optional[str] = None):
        ...
        logger.info(f"XGBoostTemperatureModel initialized with model directory: {self.model_dir}")
    ...
    def train(self, start_date: date, end_date: date, 
        ...
        """
        logger.info(f"Training XGBoost model from {start_date} to {end_date}")
    ...
    def analyze_feature_importance(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with detailed feature importance analysis
        """
        if not self.is_trained:
            raise ValueError("Model must be trained to analyze feature importance")
        ...
        for feature, importance in self.feature_importance.items():
            if 'temp' in feature.lower():
                feature_categories['temperature'].append((feature, importance))
            elif any(x in feature.lower() for x in ['humidity', 'pressure', 'wind', 'cloud', 'precipitation']):
                feature_categories['atmospheric'].append((feature, importance))
            elif any(x in feature.lower() for x in ['day', 'month', 'season', 'weekend']):
                feature_categories['date_based'].append((feature, importance))
            elif 'quality' in feature.lower() or 'missing' in feature.lower():
                feature_categories['quality'].append((feature, importance))
            elif any(x in feature.lower() for x in ['change', 'avg', 'range', 'sin', 'cos']):
                feature_categories['derived'].append((feature, importance))
            elif any(x in feature.lower() for x in ['consensus', 'agreement', 'rolling', 'trend']):
                feature_categories['ensemble'].append((feature, importance))
            elif any(x in feature.lower() for x in ['marine_layer', 'santa_ana', 'heat_island', 'fire_season']):
                feature_categories['la_patterns'].append((feature, importance))
        ...
        return {
            ...
            'top_10_features': dict(sorted(self.feature_importance.items(), key=lambda x: x[1], reverse=True)[:10]),
    ...
    def save_model(self, filename: Optional[str] = None) -> str:
        ...
        Args:
            filename: Optional filename (default: xgboost_model_YYYYMMDD.pkl)
    ...
    def validate_model_performance(self, test_start_date: date, test_end_date: date) -> Dict[str, Any]:
        ...
        logger.info(f"Validating model performance from {test_start_date} to {test_end_date}")
        
        # Prepare test data
        X_test, y_test = self.prepare_training_data(test_start_date, test_end_date)
        
        if X_test.empty or y_test.empty:
            return {'error': 'No test data available'}
        
        # Make predictions
        X_test_scaled = self.scaler.transform(X_test.fillna(X_test.median()))
        y_pred = self.model.predict(X_test_scaled)
        
        # Calculate metrics
        test_metrics = self.calculate_metrics(y_test, y_pred)
        
        # Calculate prediction errors by day
        ...
        error_analysis = {
            ...
            'days_within_1f': (errors <= 1.0).sum(),
            'days_within_2f': (errors <= 2.0).sum(),
            'days_within_3f': (errors <= 3.0).sum(),
            'total_test_days': len(errors)
        }
        ...
        return {
            ...
            'predictions_vs_actual': list(zip(y_test.tolist(), y_pred.tolist()))
        }


def main():
    """Demonstrate the XGBoost model."""
    print("=== XGBoost Temperature Model Demo ===\n")
    ...
    try:
        ...
        for i, (feature, importance) in enumerate(list(importance_analysis['top_10_features'].items())[:5]):
